...
var GLB_bar_total_time = 0;
var GLB_bar_start_time = 0;


function bar_width_func(time_ratio) {
    if (time_ratio <= 0.8) {
        return time_ratio / 1.1;
    }
    // 0.727273 when 0.8
    // 0.8x time ~ 1x time, speed 0.909091 (1/1.1) to 0.7, linear
    if (time_ratio <= 1.0) {
        var tmp = 0.727273;
        var final_speed = 0.909091 - (time_ratio - 0.8) / 0.2 * (0.909091 - 0.7);
        var tmp2 = (time_ratio - 0.8) * (0.909091 + final_speed) / 2;
        return tmp + tmp2;
    }
    // 0.8881821 when 1.0
    // 1.0x time ~ 1.15x time, speed 0.7 to 0.3, linear
    if (time_ratio <= 1.15) {
        var tmp = 0.8881821;
        var final_speed = 0.7 - (time_ratio - 1.0) / 0.15 * (0.7 - 0.3);
        var tmp2 = (time_ratio - 1.0) * (0.7 + final_speed) / 2;
        return tmp + tmp2;
    }
    // 0.96318201 when 1.15
    // after 1.15x time, speed proportional with left width, 0.3 at 1.15x time
    var tmp = 0.96318201;
    // function: 1 - n * e^(-m*t)
    // derivative at 1.15x time: m * n * e^(-mt) = mn * e^(-1.15m) = 0.3
    // value at 1.15x time: 1 - n * e^(-1.15m) = 0.96318201 => n * e^(-1.15m) = 1 - 0.96318201
    var m = 0.3 / (1 - 0.96318201);
    var n = (1 - 0.96318201) / Math.exp(-1.15 * m);
    return (1 - n * Math.exp(-time_ratio * m));
}


function update_progress_bar() {
    setTimeout(update_progress_bar, 20);
    var elapsed_time = new Date().getTime() / 1000 - GLB_bar_start_time;
    var ratio = elapsed_time / GLB_bar_total_time;
    // console.log(ratio);
    var width = bar_width_func(ratio);
    // console.log(width);
    var element = document.getElementById('bar');
    if (element === null) {
        return;
    }
    element.style.setProperty('--progress-bar-ratio', width);
}

function update_pb_wrapper() {
    setTimeout(update_pb_wrapper, 10);
    update_progress_bar();
}
...
window.addEventListener('DOMContentLoaded', function(){
    var submit = document.getElementById('gene-submit');
    submit.addEventListener('click', function(){
        ...
        if (true){
            ...
            var cell_type_checkboxes = document.querySelectorAll('.cell-types-container input');
            if (cell_type_checkboxes.length !== 14) {
                throw new Error('cell type checkboxes number is not 14');
            }
            var checked = [];
            var all_unchecked = true;
            for (var i=0; i<cell_type_checkboxes.length; i++){
                if (cell_type_checkboxes[i].checked){
                    checked.push(true);
                    all_unchecked = false;
                } else {
                    checked.push(false);
            ...
            if (all_unchecked) {
                err_msg += "Please select at least 1 cell type. ";
            }
            if (err_msg !== ''){
            ...
            var url = GLB_API_SERVER_URL + '/gene_exp/' + json_data;
            var request = new XMLHttpRequest();
            request.open('GET', url, true);
            request.timeout = 450000;
            request.responseType = 'text';
            GLB_bar_start_time = new Date().getTime() / 1000;
            text_elemant.style.display = 'none';
            document.getElementById('bar').style.display = 'block';
            if (GLB_MULTI_INPUTTED_GENES.length === 1) {
                GLB_bar_total_time = GLB_SINGLE_GENE_TIME;
            } else {
                GLB_bar_total_time = GLB_MULTI_GENE_TIME;
            }
            update_progress_bar();
            request.onload = function() {
                if (request.status === 200) {
                    ...
                    text_elemant.style.display = 'none';
                    img.src = 'data:image/png;base64,' + JSON.parse(request.responseText)['img'];
                ...
                if (request.status === 500) {
                    text_elemant.innerHTML = 'Error: ' + JSON.parse(request.responseText)['msg'];
                    document.getElementById('bar').style.display = 'none';
                    text_elemant.style.display = 'block';
                    return;
                }
